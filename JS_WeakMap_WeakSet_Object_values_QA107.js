...
// Но мы не сможем посмотреть все спрочитанные сообщения,
// weakSet не итерируемый объект, так как невохможно точно сказать сколько там элементов на текущий момент.
...
function sumSalaries(salaries) {
  ...
  let sum = 0;
  for (const num of Object.values(salaries)) {
    sum += num;
  }
  return sum;
}

cobsole.log(sumSalaries(salaries)); // 650
...
// методы для структур данных Array, Map, Set
map.keys(), map.values(), map.entries(); // возващают итерируемые объекты
...
// если нужно применить какие-то методы массивов к свойствам объектов:
